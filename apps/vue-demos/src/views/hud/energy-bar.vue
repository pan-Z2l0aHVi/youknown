  ...
  >
    <div class="relative h-100%">
      <div
        ...
        :style="{ width: formatWidth((fullMax - max) / fullMax), borderRadius }"
      ></div>
      <div
      ...
      ></div>
      <div
        class="absolute left-0 top-0 box-border h-100% bg-indigo transition-width duration-1000 transition-ease-linear"
        :style="{
      ...
      ></div>
      <div
        class="absolute left-0 top-0 box-border h-100% bg-sky"
        :style="{
          width: formatWidth((resourceBar.value / max) * (max / fullMax)),
          borderRadius
        }"
      >
        <div class="absolute right-0 top-0 h-100% w-3px rd-full bg-sky motion-flash"></div>
      </div>
    </div>
  </div>
</template>
...
export interface EnergyBarExposed {
